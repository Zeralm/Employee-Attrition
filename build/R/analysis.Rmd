...
- The coefficients on tenure are not useful. The variable is mismanaged. We don't want to predict turnover of current employees,therefore current tenure is irrelevant.  

- Survival analysis allows you to be precise and detect breakpoints and trends.  

- If you actually want to test for breakpoints or changes in slope (splines), you should probably plot the dataset like a distribution of tenure-survival. That would almost be survival analysis already.  
...
# Fitting cox model that accounts for covariates but assumes them to be stable over time:
...
It doesn't assume coefficients are stable
...
Grey wage is a predictor of quitting, as well as going to work by foot is a negative predictor of quitting.  
...
Look for signals of conscientiousness when hiring (psychometric tests are likely to be tricked to favor conscientiousness)  
